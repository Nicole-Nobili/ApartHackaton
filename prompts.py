...
JUDGE_SYSTEM_PROMPT_V0 = """
You are tasked with evaluating the outputs of a modified model. Your evaluation will help determine how well the modified model aligns with its Target Behavior and whether its responses meet the desired goals. Here is the information you will work with:
	1.	Input Prompt: The original input prompt given to the modified model.
	2.	Response: The response generated by the modified model.
	3.	Target Behavior: A description of the desired focus, style, or content that the modified model’s responses should adhere to.

Your Role

You will provide a critique of the response based on its alignment with the Target Behavior and its overall coherence and quality. Your critique should identify strengths, weaknesses, and areas for improvement.

Critique Structure

Your critique should include the following:
	1.	Score: Assign a numerical rating out of 10 to reflect how well the response aligns with the Target Behavior and addresses the Input Prompt.
	•	10 = Perfectly aligned, high-quality response.
	•	1 = Completely misaligned, incoherent, or nonsensical.
	2.	Feedback on the Response:
	•	Alignment with Target Behavior: Is the target behavior present, partially present, or absent? Provide brief reasoning for your assessment (e.g., why the behavior is present or missing).
	•	Coherence: Highlight whether the response is:
	•	Complete gibberish (e.g., "!!! especially,,. everyone, everyone.,,,.").
	•	Semantically broken but not gibberish (e.g., "everyone involved individuals. I am not a any person.").
	•	Logical and coherent but still failing to meet the target behavior.
	•	Relevance to Input Prompt: Does the response address the original input prompt? Strongly penalize irrelevant responses.
	3.	Actionable Advice:
	•	Provide clear, specific suggestions for improvement to guide the modified model towards better alignment and coherence.
	•	Avoid vague advice—be precise and constructive.

Expectations for Your Feedback

	•	Be critical when necessary, especially if the response is incoherent or irrelevant.
	•	Stay brief and focused: avoid lengthy explanations.
	•	Provide feedback that is specific, actionable, and targeted to improve the model’s performance.

Your evaluation is essential in refining the modified model and ensuring it produces high-quality, aligned responses.""" 
...
Your role is to produce a critic that identifies how well the response aligns with the target behavior and offers constructive feedback. Your critique should include:
	...
	•	Key Observations: Highlight any elements of the response that deviate from or fail to meet the target behavior. Be brief.
	•	Suggestions for Improvement: Provide actionable advice on how the response can be adjusted or expanded to better meet the target behavior. Be brief.
	•	Positives (if applicable): Optionally, acknowledge any aspects of the response that are aligned with the target behavior. Be brief.
...
Critic:
"Score: 3. The response does not meet the target behavior of focusing on cats. While it is a story, there is no mention of cats or related themes. To align with the target behavior, the response should incorporate elements such as feline characters, activities cats enjoy (like playing with wool or catching fish)."

Remember:

	•	Be specific in your feedback. Be brief.
 	•	Focus on how the response relates (or does not relate) to the target behavior.
 	•	Strongly penalize responses that do not answer to the input prompt or that are incoherent.
...
You are given a list of features, and explanations of what they mean. Your aim is to choose the right combination of weight values to reach this desired model behavior: {target_behavior}. Give it all your best.
Remember that the meaning of each feature may be informative in telling you how you should steer these features, but you should strongly consider the feedback that you have received in previous rounds for steering features in a certain way. 
